...
function render(food) {
    const foodCard = document.getElementById('div');
    foodCard.classList.add('card');
    const foodName = document.getElementById('h2');
    foodName.textContent = food.name;
    const foodImage = document.getElementById('img');
    foodImage.src = food.img;
    foodCard.append(foodName, foodImage);
    return foodCard;
}

for (let food of foods) {
    const foodElements = render(food);
    foodList.appendChild(foodElements);
}